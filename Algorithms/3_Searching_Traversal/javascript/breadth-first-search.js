...
class BinarySearchTree {
  ...
  breadthFirstSearchIterative() {
    ...
    let list = [];
    let queue = [];
    queue.push(currentNode); //push the first node (number 9)

    while (queue.length > 0) {
      currentNode = queue.shift(); //return and remove 1st item from the queue - (number 9 in the first iteration, 4 in the next iteration)
      list.push(currentNode.value); //(number 9 in the first iteration, 4 in the next iteration)

      if (currentNode.left) {
        queue.push(currentNode.left); //(number 4 in the first iteration, 1 in the next iteration)
      }
      if (currentNode.right) {
        queue.push(currentNode.right); //(number 20 in the first iteration, 6 in the next iteration)
      }
    }
    return list;
  }
